...
export const ZoomSection = () => {
    ...
    return (
        <Box h={'190vh'} as='section' pos={'relative'} bg="#000">
            <AnimatePresence>
                <Box pos={'relative'} w='full' h='65%'>
                        ...
                        <Stack mt={20} spacing={16} className='grid-container' maxW={'5xl'} mx={'auto'} pos={'absolute'} left={'50%'} w='full' overflow={'hidden'}>
                            <Box h={'auto'} overflow={'hidden'}>
                                <video ref={videoRef} autoPlay muted playsInline>
                                    <source src='./src/assets/videos/zoom.mp4' type="video/mp4" style={{ objectFit: 'cover', }} />
                                </video>
                                <Text cursor={'pointer'} onClick={() => videoRef.current?.play()} color={'#86868b'} textAlign={'center'} fontSize={'xl'} fontWeight={600} mt={4}>
                                    Replay
                                </Text>
                            </Box>
                            <HStack ref={ref} spacing={{ base: 10, md: 20 }} alignItems={'start'} justifyContent={'space-around'}>
                                <Box maxW={'33%'}>
                                    {isInView2 &&
                                ...
                                <Box maxW={'33%'}>
                                    {isInView2 && <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: 'easeInOut', delay: 0.5 }}>
                                        <Stack spacing={0}>
                                            <Text color={'#fff'} fontSize={'5xl'} fontWeight={600} lineHeight={'120%'}>5x optical</Text>
                                            <Text color={'#fff'} fontSize={'5xl'} fontWeight={600} lineHeight={'120%'}>Zoom</Text>
                                            <Text color={'#86868b'} fontSize={'xl'} fontWeight={600}>with the 120 mm lens</Text>
                                        </Stack>
                                    </motion.div>}
                                </Box>
                            </HStack>
                    ...
                    <SectionButton isInView={isInView2} width='300px'>
                        <HStack h={'58px'} px={4} justifyContent={'space-between'} alignItems={'center'}>
                            <motion.div style={{ width: '100%' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, ease: 'easeInOut', delay: 1.5 }}>
                                <HStack w={'full'} justifyContent={'space-between'} spacing={2}>
                                    <Text color={'white'} fontSize={'lg'} fontWeight={500}>Nerd out on 5x Telephoto</Text>
